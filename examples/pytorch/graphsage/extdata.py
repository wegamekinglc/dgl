...
class ExtDataset(DGLBuiltinDataset):
    ...
    def process(self):
        # graph
        coo_adj = sp.load_npz(os.path.join(self.raw_path, "graph.npz"))
        self._graph = from_scipy(coo_adj)
        # features and labels
        ext_data = np.load(os.path.join(self.raw_path, "data.npz"))
        features = ext_data["feature"]
        labels = ext_data["label"]
        # tarin/val/test indices
        node_types = ext_data["node_types"]
    ...
    def __getitem__(self, idx):
        ...
        """
        assert idx == 0, "External Dataset only has one graph"
        return self._graph
